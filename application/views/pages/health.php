	...
	<style>
		...
		<script>
          ...
          if(posts_num >= 2){
              ...
              console.log(offset);
              $.ajax({
                url : "<?php echo site_url('meetglobal/load_more_posts_front_health') ?>",
                type : "POST",
                responseType : "json",
                dataType : "json",
                async : false,
  ...
  overflow-y: hidden;
          ...
          <div class="">
            <div class="">
              <div class="text-center posts-container">
                <?php 
                $posts_by_user = $this->meetglobal_model->getFirstTenPostsByUserFrontHealth($user_id);
                
                if(is_array($posts_by_user) && count($posts_by_user) > 0){
              ...
              <?php } }else{  ?> 
              	<h3 class="text-warning">No Health Posts To Display</h3>
              <?php } ?>
              </div>
      ...
      <div class="modal fade" id="makePostModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h3 class="modal-title" id="exampleModalLabel">Make Health Post</h3>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
      ...
      <script>
        $(document).ready(function() {
          ...
          $('#makePostModal .modal-body').fineUploader({
              ...
              request: {
                  endpoint: '<?php echo site_url('meetglobal/index/upload_post_images_health'); ?>',
                  params : {
                      'upload_image' : true
                  }
              },
              ...
              callbacks:  {
                  ...
                  onAllComplete : function (id,name,response,xhr) {
                    ...
                    function testFunc () {
                       $.ajax({
                        url : "<?php echo site_url('meetglobal/get_post_health'); ?>",
                        type : "POST",
                        responseType : "json",
                        dataType : "json",
                        data : "post_id="+post_id,
          ...
          $('#upload').click(function() {
            ...
            $(".spinner-overlay").show();
            $.ajax({
              url : "<?php echo site_url('meetglobal/index/make_post_health'); ?>",
              type : "POST",
              responseType : "json",
              dataType : "json",
              data : "post="+post,
